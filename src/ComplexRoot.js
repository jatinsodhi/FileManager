...
class SortColumn {
    ...
    constructor(column = 'name', order = 'asc') {
        this.column = column;
        this.order = order;
    }
    ...
    static setState(sortColumn) {
        const { column, order } = sortColumn;
        let item = null;
        let json = localStorage.getItem(SortColumn.storageName);
    ...
    static getOrder(column) {
        return SortColumn.getState(column).order;
    }

    static getSign(column) {
...
export const ComplexRoot = ({
    ...
    const handleSorting = useCallback((a, b) => {
